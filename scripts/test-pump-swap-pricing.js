...
// SWOLEDOGE regression: raw vault pricing was 13.18% below PumpSwap's
// effective-reserve price after virtual_quote_reserves was enabled.
const state = {
  poolBaseAmount: { toString: () => '100000000000000' }, // 100M tokens
  poolQuoteAmount: { toString: () => '135800000000' },   // raw 135.8 SOL
  pool: {
    virtualQuoteReserves: { toString: () => '17900000000' }, // 17.9 SOL
  },
};
const rawQuoteBefore = state.poolQuoteAmount.toString();
...
assert.strictEqual(
  ...
  rawQuoteBefore,
  'pricing must not mutate or double-adjust the raw vault reserve',
);
...
const parseFullVault = () => detector._parseFullVault(
  {},
  {},
  [
...
let refreshCalls = 0;
detector._poolRefreshRequests = new Map();
detector._poolRefreshRetryMs = 1000;
...
assert.strictEqual(parseFullVault(), null, 'cache miss must skip instead of using raw pricing');
assert.strictEqual(parseFullVault(), null, 'repeated cache miss must still skip the raw price');
assert.strictEqual(refreshCalls, 1, 'cache-miss refresh requests must be deduplicated');
