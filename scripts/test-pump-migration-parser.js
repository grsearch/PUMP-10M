...
const parsedJsonV2Keys = [PUMP_PROGRAM_ID, ...compiledV2Accounts].map((pubkey, index) => ({
  pubkey,
  signer: false,
  writable: index > 0,
  source: index <= 11 ? 'transaction' : 'lookupTable',
}));
const jsonParsedV2 = {
  slot: 123458,
...
assert(parsedJsonV2, 'jsonParsed V2 indexes must resolve from ALT-expanded accountKeys without loadedAddresses');
assert.strictEqual(parsedJsonV2.poolBaseVault, key('V'));
assert.strictEqual(parsedJsonV2.poolQuoteVault, key('W'));
assert.notStrictEqual(parsedJsonV2.poolQuoteVault, TOKEN_2022_PROGRAM_ID);
